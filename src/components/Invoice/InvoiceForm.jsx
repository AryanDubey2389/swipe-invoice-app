...
const notifyError = (msg) => toast.error(`${msg}`, { position: toast.POSITION.TOP_CENTER });
const InvoiceForm = () => {
  ...
  const onItemizedItemEdit = (evt, id) => {
    ...
    const updatedItems = formData.items.map((oldItem) => {
      if (oldItem.itemId === id) {
        // dispatch(updateProductDetails({ id, updatedDetails: item }));
        if (evt.target.name == "itemId") {
          const item = getOneProduct(evt.target.value);
          return {
            ...oldItem,
  ...
  const handleAddInvoice = () => {
    if (isEdit) {
      notify("Invoice updated successfuly 🥳");
      dispatch(updateInvoice({ id: params.id, updatedInvoice: formData }));
    } else if (isCopy) {
      dispatch(addInvoice({ id: generateRandomId(), ...formData }));
      notify("Invoice added successfuly 🥳");
    } else {
